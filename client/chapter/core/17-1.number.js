...
let getRandomMinMax = (min, max) => {
  if (min > max) throw new Error('min이 max보다 클 수 없습니다.');
  // (max-min) -> 범위의 너비
  // Math.random() * (max-min) -> 0부터 (min-max)까지의 랜덤 소수 값
  // Math.random() * (max-min)+min -> 시작 숫자를 min부터로 맞추기 위해 더해줌
